...
def render_page():
    ...
    # Container da prévia (reativo e dinâmico)
    preview_container = ui.column().classes('w-full q-pa-none gap-4')

    def render_preview():
        preview_container.clear()
        if not import_state['dados_novos']:
            return
            
        with preview_container:
            with theme.card_base().classes('w-full p-6'):
                ui.label(f"PRÉVIA DOS DADOS DO CADASTRO (ANO LETIVO DE DESTINO: {import_state['ano_letivo']})").classes('cyber-title text-sm font-bold text-white q-mb-md')
                ...
                if len(import_state['dados_novos']) > 5:
                    ui.label(f"... e mais {len(import_state['dados_novos']) - 5} alunos no lote de cadastro.").classes('text-caption text-grey-5 italic q-mb-md')
                ...
                async def executar_importacao():
                    ...
                    try:
                        ...
                        ui.notify(f"🎉 Importação de Alunos concluída! {sucessos} cadastros processados com sucesso.", color='positive')
                ...
                with ui.row().classes('w-full justify-end gap-2'):
                    ...
                    ui.button(
                        f'CONFIRMAR IMPORTAÇÃO E SALVAR NO BANCO DE DADOS', 
